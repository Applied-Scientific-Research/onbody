...
int main(int argc, char *argv[]) {
    ...
    Parts<STORE,ACCUM,3,3,3> srcs(numSrcs);
    // initialize particle data
    srcs.random_in_cube();
    //srcs.smooth_strengths();
    srcs.wave_strengths();

    Parts<STORE,ACCUM,3,3,3> targs(numTargs);
    // initialize particle data
    ...
    start = std::chrono::system_clock::now();
    Parts<STORE,ACCUM,3,3,3> eqsrcs((stree.numnodes/2) * blockSize);
    ...
    Parts<STORE,ACCUM,3,3,3> eqtargs(0);
    if (test_iterations[4] > 0) {
        printf("\nCalculating equivalent targ points\n");
        start = std::chrono::system_clock::now();
        eqtargs = Parts<STORE,ACCUM,3,3,3>((ttree.numnodes/2) * blockSize);
        printf("  need %ld particles\n", eqtargs.n);
        end = std::chrono::system_clock::now(); elapsed_seconds = end-start;
