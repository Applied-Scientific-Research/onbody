...
int main(int argc, char *argv[]) {
    ...
    Parts<STORE,ACCUM,3,3,3> srcs(numSrcs, true, blockSize);
    ...
    Parts<STORE,ACCUM,3,3,3> targs(numTargs, false, blockSize);
    // initialize particle data
    ...
    Parts<STORE,ACCUM,3,3,3> eqsrcs(0, true, eqBlockSize);
    if (make_source_tree) {
    ...
    Parts<STORE,ACCUM,3,3,3> eqtargs(0, false, eqBlockSize);
    if (arrange_target_parts) {
        printf("\nCalculating equivalent targ points\n");
        start = std::chrono::steady_clock::now();
        eqtargs = Parts<STORE,ACCUM,3,3,3>((ttree.numnodes/2) * eqtargs.blockSize, false);
        printf("  need %ld particles\n", eqtargs.n);
        end = std::chrono::steady_clock::now(); elapsed_seconds = end-start;
