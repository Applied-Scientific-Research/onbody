/*
 * interface2dvorttr.cpp - interface code for Barnes-Hut treecode, target have radii
 *
...
//
extern "C" float external_vel_solver_tr_f_ (const int* nsrc,
    ...
    for (int i=0; i<*ntarg; ++i) targs.r[i] = tr[i];
...
//
extern "C" float external_vel_direct_tr_f_ (const int* nsrc,
                                         const float* sx, const float* sy,
                                         const float* ss, const float* sr,
                                         const int* ntarg,
                                         const float* tx, const float* ty,
